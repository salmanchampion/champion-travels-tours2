      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
            ...
            <div className="mb-16 p-6 md:p-8 bg-[var(--color-dark-bg)] rounded-[var(--ui-border-radius)] border border-gray-700/50 relative overflow-hidden shadow-lg" data-aos="fade-up">
                ...
                <div className="relative z-10 flex flex-col lg:flex-row items-center justify-between gap-6 md:gap-8">
                    <div className="text-center lg:text-left w-full lg:w-auto">
                        ...
                        <p className="text-[var(--color-muted-text)] max-w-xl text-sm md:text-base">{t(newsletter.subtitle)}</p>
                    </div>
                    <form onSubmit={handleSubscribe} className="w-full lg:w-auto flex flex-col sm:flex-row gap-3">
                        ...
                        >
                            {status === 'submitting' ? 'Submitting...' : status === 'success' ? 'Subscribed!' : status === 'error' ? 'Failed' : t(newsletter.buttonText)}
                        </button>
                    </form>
                </div>
            </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 md:gap-12">
          ...
          <div className="sm:col-span-2 lg:col-span-1 text-center sm:text-left">
             <a href="#home" className="block mb-4">
                <span className="font-display text-3xl font-bold tracking-tight">
                    <span className="text-[var(--color-light-text)]">{t(footer.about.title[0])}</span>
                    <span className="text-[var(--color-primary)]"> {t(footer.about.title[1])}</span>
                </span>
            </a>
          ...
          <div className="text-center sm:text-left">
            ...
            <ul className="space-y-3">
              {visibleQuickLinks.map(link => (
                  <li key={link.href}><a href={link.href} className="text-[var(--color-muted-text)] hover:text-[var(--color-primary)] transition-colors block py-1 text-sm md:text-base">{t(link.label)}</a></li>
              ))}
            </ul>
          </div>
          ...
          <div className="text-center sm:text-left">
            ...
            <p className="text-[var(--color-muted-text)] mb-6 text-sm md:text-base">{t(footer.followUs.description)}</p>
